...
const cssFrom = html.indexOf('/* \u2500\u2500 Huddles, group panel & status chips \u2500\u2500 */');
const mqAt = html.indexOf('@media (max-width:640px) {', cssFrom);
let d = 0, cssEnd = mqAt;
for (; cssEnd < html.length; cssEnd++) {
  if (html[cssEnd] === '{') d++;
  else if (html[cssEnd] === '}') { d--; if (!d) { cssEnd++; break; } }
}
const CSS = html.slice(cssFrom, cssEnd);
